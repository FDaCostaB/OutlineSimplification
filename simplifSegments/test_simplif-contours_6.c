...
int main(int argc,char *argv[]){
    Image I;
    sequence_tab_seg *res = (sequence_tab_seg *) malloc(sizeof(sequence_tab_seg));
    init_sequence_tab_segment(res);
    sequence_tab_point tab_contours;
    init_sequence_tab_point(&tab_contours);
    FILE *f;

    if(argc != 4 ) ERREUR_FATALE("Utilisation commande : ./test_image fichier_image fichier_resultat distanceSeuil\n");
    ...
    res = simplif_pls_contours(&tab_contours,d);

    afficher_contours_simple(res,stdout);
    dessiner_pls_seg(res,I.L,I.H,f);
    fclose(f);
    free(res);
}
