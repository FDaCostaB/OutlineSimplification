...
int main(int argc,char *argv[]){
    Image I;
    sequence_tab_bezier3 *res = (sequence_tab_bezier3 *) malloc(sizeof(sequence_tab_bezier3));
    init_sequence_tab_bezier3(res);
    ...
    res = simplif_pls_contours_bezier3(&tab_contours,d);

    afficher_contours_bezier3_simple(res,stdout);
    dessiner_pls_bezier3(res,I.L,I.H,f);
    fclose(f);
}
...
/*tableau_point tab_point;
    ...
    bezier3 courbe3 = approx_contour_b3(&tab_point,0,tab_point.longueur-1);
    printf("La courbe de bézier 3 correspondant : C0 : (%f,%f) C1: (%f,%f) C2 : (%f,%f) C3 : (%f,%f)\n",
           courbe3.C0.x, courbe3.C0.y, courbe3.C1.x, courbe3.C1.y, courbe3.C2.x, courbe3.C2.y, courbe3.C3.x, courbe3.C3.y);
    for(int i=1;i<tab_point.longueur-1;i++){
        double lambda = dist_point_bezier3( (double)i/((double)tab_point.longueur - 1),tab_point.tab[i],courbe3);
        printf("\nd(P_i , c(t_i)) = %f \n",lambda);
    }*/
