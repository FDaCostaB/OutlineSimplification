...
void dessiner_pls_bezier3(sequence_tab_bezier3 *seq, int xmax, int ymax, FILE *f){
    bezier3 courbe3;
    cellule_tab_bezier3 *curr=seq->tete;
    Point p= curr->tab_bezier3->tab[0].C0;
    en_tete(xmax,ymax,f);
    while (curr != NULL){
        fprintf(f,"%d %d moveto\n", (int)p.x, ymax - (int)p.y);
        for(int i = 0; i <curr->tab_bezier3->longueur; i++){
            courbe3=curr->tab_bezier3->tab[i];
            courbe3.C0.y = ymax - courbe3.C0.y;
            courbe3.C1.y = ymax - courbe3.C1.y;
            courbe3.C2.y = ymax - courbe3.C2.y;
            courbe3.C3.y = ymax - courbe3.C3.y;
            creer_bezier3(courbe3, f);
        }
        curr = curr -> suivant;
        if(curr != NULL) p=curr->tab_bezier3->tab[0].C0;
    }
    fprintf(f,"fill\n");
    fprintf(f,"showpage\n");
}